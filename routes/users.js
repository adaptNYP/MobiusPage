...
router.get('/register_admin', ensureAuthenticated, (req, res) =>{
	if (req.user.admin == true){
		res.render('register_admin', { LoginUser: req.user, name: req.user.name, email: req.user.email, school: req.user.email, admin: req.user.admin});
	}
	else{
...
router.post('/register_admin', ensureAuthenticated, async (req, res) =>{
	const { name, email, school, admin} = req.body;
	var is_admin = false;
	if (admin === "admin"){
		is_admin = true;
	}
